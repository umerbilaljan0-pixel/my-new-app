export function Logo({ size = 18 }: { size?: number }) {
  const tri = size * 0.62;
  return (
    <span className="inline-flex items-baseline font-sans font-semibold select-none"
      style={{ fontSize: size, letterSpacing: "-0.03em", color: "var(--text-hi)" }} aria-label="CLEANPLATE">
      CLEANPL
...
export function Monogram({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" aria-label="CLEANPLATE">
      <rect width="32" height="32" rx="4" fill="var(--bg-void)" stroke="var(--border)" />
